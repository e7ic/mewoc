  ...
  return new Promise((resolve, reject) => {
    ...
    transaction.oncomplete = () => {
      ...
      for (const { asset, request } of requests) {
        if (!request.result) {
          reject(new Error(`图片「${asset.fileName}」的本地资源缺失`))
          return
        }
    ...
    transaction.onerror = () => reject(new Error("读取文档图片失败，请重试"))
    transaction.onabort = () => reject(new Error("文档图片读取已中止，请重试"))
  })
}
    ...
    request.onsuccess = () => {
      ...
      try {
        ...
        for (const asset of document.assets) {
          ...
          if (!entry?.blob || entry.blob.size !== asset.byteLength) {
            failure = new Error(`图片「${asset.fileName}」尚未就绪，文档未保存`)
            transaction.abort()
            return
          }
          // 资源 ID 在便携文件内稳定；数据库按文档隔离，导入变体不会覆盖其他文档的图片。
